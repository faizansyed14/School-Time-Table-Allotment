...
def run_phase_b(allocations, targets, teachers, classes, CT, LVL, mp, time_limit, workers,
    ...
    # ── Best-effort SAME-PERIOD consistency (soft) ──────────────────────
    # Goal: keep the same (teacher, subject) block in the same period across
    # all days, so a teacher follows a fixed daily routine. Strict fixed
    # columns are impossible when a subject's weekly total isn't a multiple
    # of NUM_DAYS, so this is modelled as an objective (minimise the number
    # of distinct (class, period, teacher+subject) blocks) rather than a hard
    # rule. All of R1–R5 above remain hard constraints.
    if consistency:
        units_by_c_key = {}
        for i, (t, c, s) in enumerate(units):
            units_by_c_key.setdefault((c, (t, s)), []).append(i)
        present = []
        for (c, _key), idxs in units_by_c_key.items():
            for p in range(PPD):
                zv = M.new_bool_var("")
                # zv must be 1 if this block uses period p on any day.
                M.add(sum(X[(i, d, p)] for i in idxs for d in range(NUM_DAYS)) <= len(idxs) * zv)
                present.append(zv)
        # Fewer distinct blocks per period column ⇒ more consistent routine.
        M.minimize(sum(present))
...
def solve(data, time_limit=60, workers=8):
    ...
    consistency = bool(data.get("same_period_consistency", True))
    ...
    if mode == "schedule":
        ...
        return run_phase_b(allocations, targets, teachers, classes, CT, LVL, mp, time_limit, workers, consistency)

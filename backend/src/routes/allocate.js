...
// POST /run — saved allocations → CP-SAT Phase B (schedule) → preview grid
router.post('/run', async (_req, res) => {
  try {
    const startTime = Date.now();

    const [subjectsRes, classesRes, teachersRes, allocsRes] = await Promise.all([
      ...
      supabase.from('subject_allocations')
        .select('teacher_id, class_id, subject, periods_weekly, teachers(name), classes(name)'),
    ]);
    ...
    const dbAllocRows = (allocsRes.data || []).map((r) => ({
    ...
    const planIssues = buildAllocationPlanIssues({ subjects, classes, teachers, allocations: dbAllocRows });
    ...
    if (planErrors.length > 0) {
      return res.json({
        ...
        ok: false,
        phase: 'schedule',
        message: planErrors[0].message,
        errors: planErrors.map((i) => ({ type: i.type, message: i.message })),
        issues: planIssues,
        filled: 0,
        total: classes.length * 48,
      });
    }

    const solverAllocations = mapDbAllocationsToSolver(dbAllocRows, teachers, classes);
    const payload = buildSolverPayload({
      teachers, classes, subjects, mode: 'schedule', allocations: solverAllocations,
    });

    console.log('[CP-SAT] Scheduling saved allocations (Phase B)…');
    const raw = await runSolver(payload);
    const result = transformSolverResult(raw, { teachers, classes });
    result.mode = 'schedule';
    result.phase = raw.phase || 'schedule';
...
router.post('/apply', async (req, res) => {
  try {
    ...
    if (result.teacher_summary) {
      for (const [tid, data] of Object.entries(result.teacher_summary)) {
        await supabase.from('teachers').update({ allocated_periods: data.allocated || 0 }).eq('id', tid);
      }
    }
